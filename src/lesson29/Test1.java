...
public class Test1 {
   /* public static void main(String[] args) {
        ...
        list.add(s5);
        list.add(s6);
        StudentShowInfo.studentOverGrade(list,4.2);
        System.out.println();
        StudentShowInfo.studentsMixCondition(list, 'f', 20);
    }
}
class Student{
    String name;
...
class StudentShowInfo{
   ...
   static void studentOverGrade(ArrayList<Student> al, double avgGrade){
        for(Student s : al){
  ...
  static   void studentsMixCondition(ArrayList<Student> al, char sex, int age){
        for(Student s : al){
            if(s.sex=='f' && s.age<20){
                showInfo(s);
            }
